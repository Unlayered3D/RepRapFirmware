...
// If PID parameters are overridden, append a M301 command for this heater followed by a newline to the string
...
void FopDt::CalcPidConstants(float targetTemperature) noexcept
{
	if (pidParametersOverridden)
	{
		return;					// PID parameters have been set by M301, don't overwrite them
	}
